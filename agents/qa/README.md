...
5. New minors: cans-wins conflict detection is similarity-heuristic (divergent edit re-imports as duplicate sibling, `conflicts:[]`); `budget read --change <unknown>` silently ignored; `new task` duplicate-of-pristine-template returns ok with no write; `init --force` clobbers user-edited spec files per §21's letter while now protecting `_collab/` (destructive scope undocumented); §16 content knobs (`tbd_allowed`, `max_tbd_per_file`) inert; whitespace-only file containing a tab line → parse error exit 1 instead of §11's "non-bullet lines ignored" (QA-15).
6. PERF (known, matches source): 10k-node `cans check` ≈ 7.95 s text / ≈ 71 s `--json` on the npm binary — within the documented §13 O(N²) envelope and ≡ round-2 source numbers (8.4 s / 69 s), but far past §40's "no operation exceeds 500 ms" stance; normal workspaces clear every §40 row (532 nodes: check 96 ms, json 98 ms, status 27 ms, budget 25 ms).
7. Package integrity (QA-15, filename-level only): tarball clean — 34 entries, all templates present, no lockfile/node_modules/test; registry metadata matches §39 except §39's stale `"name": "cans"` (actual `cans-spec`) and `src/core/args.ts` missing from §9's map. Symlink handling remains untested on any artifact (sandbox denies `ln -s`).
